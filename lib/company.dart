...
import 'shared_pref_helper.dart'; // Ensure this helper exists
import 'company_item.dart'; // Ensure this file exists
import 'Login.dart'; // Ensure you have this LoginPage widget defined
import 'cart.dart';
import 'order.dart';
import 'mycart.dart';
...
class CompanyPage extends StatefulWidget {
  ...
  const CompanyPage({super.key, required this.userid});

  @override
  _CompanyPageState createState() => _CompanyPageState();
}

class _CompanyPageState extends State<CompanyPage> with WidgetsBindingObserver{
  List userData = [];
...
void dispose() {
  ...
  _searchController.dispose(); // Dispose of the search controller
   // Remove the observer
  _stopListening(); 
  WidgetsBinding.instance.removeObserver(this);// Stop listening to speech recognition
...
void didChangeAppLifecycleState(AppLifecycleState state) {
  if (state == AppLifecycleState.resumed) {
    // Reinitialize speech to text when the app 
    fetchCartQty();
    _initSpeech();
...
void _onSpeechResult(SpeechRecognitionResult result) {
  setState(() {
    ...
    _searchController.text = _lastWords; // Set search bar text to speech result
  });
...
//       );
      case 4:
        if (showOrder) {
          ...
          Navigator.pushReplacement(
            context,
            MaterialPageRoute(builder: (context) => OrderMasterByCustId(userid:widget.userid)),
          );
        }
...
bool showCart = false;
 ...
 Future<void> fetchUserTypeAndSetVisibility() async {
  try {
    globals.usertype = await SharedPrefHelper.getUsertype() ?? 'default'; // Set a default if null
              ...
              child: userData.isEmpty
                  ...
                  : GridView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        String imageUrl = '${globals.baseImageUrl}${item['ic_image']}';
                        return categoryCard(
                          item['ic_name'],
                          imageUrl,
                          item['ic_id'] ?? '',
  ...
  Widget categoryCard(String name, String imagePath, String icId) {
          ...
          MaterialPageRoute(
            builder: (context) => CompanyItem(
              ...
              ig_custid: globals.userid!,
               userid:globals.userid!, // Passing ig_id from userData
            ),
          ),
        );
      },
                ...
                image: DecorationImage(
                  ...
                  image: imagePath.startsWith('http')
                      ...
                      : AssetImage(imagePath)
                          as ImageProvider, // Use AssetImage for local files
                  fit: BoxFit.contain,
                ),
              ),
            ),
