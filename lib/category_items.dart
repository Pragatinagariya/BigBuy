  ...
  void initState() {
    ...
    _searchController.addListener(() {
      if (_searchController.text.isNotEmpty) {
        searchItems(_searchController.text, grpno);
      } else {
...
Future<void> fetchCartQty() async {
 ...
 Future<void> searchItems(String query, String grpno) async {
  // Only proceed if query and grpno are not empty
  if (query.isEmpty || grpno.isEmpty) {
    return; // Don't proceed if empty query or grpno
  }
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      if (jsonResponse is List) {
        setState(() {
          // Update searchResults dynamically
...
//       );
 ...
 Future<void> addToCart(String itemId, String qty, String rate) async {
  ...
  void filterSearchResults(String query, String grpno) {
    if (query.isNotEmpty) {
      searchItems(query, grpno); // Call the search API
    } else {
      setState(() {
        filteredData = userData; // Show all data if the search is empty
      });
    }
  }
  ...
  Future<void> getRecord() async {
    String uri =
        "${globals.uriname}item.php?ig_id=${widget.ig_id}&ig_custid=${globals.userid!}"; // Include clientcode in the URL
    try {
      var response = await http.get(Uri.parse(uri));
                  ...
                  suffixIcon: Padding(
                    ...
                    child: Wrap(
                      ...
                      onPressed: () {
                        ...
                        String grpno =
                            widget.ig_id; // Ensure this is the correct value

                        print(
                            'Navigating to SearchResultPages with Query: $query, Group No: $grpno');
                        Navigator.push(
                          context,
                          MaterialPageRoute(
                            builder: (context) => SearchResultPages(
                              contextquery: _searchController.text,
                              contextig_id: widget.ig_id,
...
Padding(
  ...
  //                 ? searchResults[index]
  //                 : filteredData[index]; // Ensure all three lists are considered
  child: _searchController.text.isNotEmpty && searchResults.isEmpty
      ...
      : ListView.builder(
          itemCount: _searchController.text.isEmpty
              ? (filteredData.isNotEmpty ? filteredData.length : userData.length)
              : (searchResults.isNotEmpty ? searchResults.length : 0), // Set 0 when search term exists but no results
          itemBuilder: (context, index) {
            final currentItem = _searchController.text.isEmpty
                ? (filteredData.isNotEmpty ? filteredData[index] : userData[index])
                : searchResults[index];
  ...
  MaterialPageRoute(
    builder: (context) => CartPage(
      ...
      imageUrl: currentItem["im_image"] != null &&
              currentItem["im_image"].isNotEmpty
  ...
  if (updatedData != null) {
    setState(() {
      currentItem["im_cartqty"] = updatedData["im_cartqty"].toString(); // Ensure it's a string
      currentItem["im_rate"] = updatedData["im_rate"].toString(); // Ensure it's a string
    });
  }
});
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              currentItem["im_cartqty"] != null &&
                                      int.tryParse(currentItem["im_cartqty"]) !=
                                          null &&
                                    ...
                                    builder: (BuildContext context) {
                                      TextEditingController quantityController =
                                        ...
                                        text: currentItem["im_cartqty"] != null &&
                                                int.tryParse(currentItem["im_cartqty"]) !=
                                                    null
                                            ? currentItem["im_cartqty"]
                                                .toString() // Show existing quantity
                                            : '', // Set to empty string for no default value
                                      );
                                      ...
                                      return AlertDialog(
                                        ...
                                        content: TextField(
                                          ...
                                          focusNode: quantityFocusNode, // Attach the FocusNode
