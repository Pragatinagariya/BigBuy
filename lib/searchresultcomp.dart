...
class SearchResultComp extends StatefulWidget {
  final String contextquery;
  final String userid;

    final String contextic_id;

  const SearchResultComp({super.key, required this.contextquery, required this.userid,required this.contextic_id});

  @override
  _SearchResultCompState createState() => _SearchResultCompState();
}

class _SearchResultCompState extends State<SearchResultComp> {
  List searchResults = [];
...
void initState() {
  ...
  String dynamicQuery = widget.contextquery;
   // Assume this comes from a widget property or user input
  String dynamicCmpname = widget.contextic_id; // Same for grpno

  someFunction(dynamicQuery, dynamicCmpname);
}

void someFunction(String query, String Cmpname) {
  print('Before calling searchItems: Query: $query, Group No: $Cmpname');
  // Call searchItems with the specified query and group number
  searchItems(query, Cmpname);
}

 Future<void> searchItems(String query, String cmpname) async {
    print('Query: $query, Company Name: $cmpname');
    String uri = "${globals.uriname}Search.php?name=$query&flag=cmp&cmpname=${cmpname.trim()}"; // Updated URL parameter to use company name
    print('API URI: $uri');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (jsonResponse is List) {
          setState(() {
            // Store the results returned from the API
            searchResults = jsonResponse;
            print('Filtered Results: $searchResults');
          });
        }
      } else {
        print('Error fetching data: ${response.statusCode}'); // Log errors if status is not 200
      }
    } catch (e) {
      print('Error fetching search results: $e'); // Log any exceptions
    }
  }
                                  ...
                                  MaterialPageRoute(
                                    builder: (context) => CartPage(
                                      ...
                                      imageUrl: searchResults[index]["im_image"] != null &&
                                          ...
                                          ? '{baseImageUrl}${searchResults[index]["im_image"]}'
                              ...
                              child: SizedBox(
                                ...
                                child: Image.network(
                                  searchResults[index]["im_image"] != null &&
                                          searchResults[index]["im_image"].isNotEmpty
                                      ? '${globals.baseImageUrl}l${searchResults[index]["im_image"]}'
                                      : '{baseImageUrl}0000000.jpg',
                                  fit: BoxFit.contain,
                                ),
                              ),
                                  ...
                                  Expanded(
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        // Text('${searchResults[index]["im_Qty"] ?? "N/A"}', style: const TextStyle(fontSize: 14, color: Colors.teal)),
